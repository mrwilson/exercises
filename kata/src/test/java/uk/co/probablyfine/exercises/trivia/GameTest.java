...
public class GameTest {
    ...
    public static void main(String... args) throws FileNotFoundException {
        ...
        System.setOut(new PrintStream(new FileOutputStream(fixedOutput)));

        List<Integer> rolls = new ArrayList<>();

        BoundedRandomness recorder =
                maximum -> {
        ...
        String collect = rolls.stream().map(Object::toString).collect(Collectors.joining(","));
    ...
    public void runFixedOutput() throws IOException {
        ...
        ListIterator<Integer> inputs =
                Arrays.stream(fixedRun.get(fixedRun.size() - 1).split(","))
                        .map(Integer::parseInt)
                        .collect(Collectors.toList())
                        .listIterator();

        BoundedRandomness randomness =
                (i) -> {
                    if (inputs.hasNext()) {
                        return inputs.next();
                    } else {
    ...
    public void questionCategoryIsPopWhenPlayerPlaceIs0Modulo4() {
        ...
        List.of(0, 4, 8)
                .forEach(
                        place -> {
                            Player one = new Player("player", place, 0, false);
